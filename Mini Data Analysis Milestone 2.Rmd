...
I will create a graph with year_level (the same variable that I made in the above summarizing task) on the x-axis and no_of_accessible_parking_spaces on the y-axis. I will use geom_boxplot() and geom_jitter().
...
I will be creating a graph using geom_jitter() and geom_smooth(), incorporating the variable created above, _rise_level_.
...
For this task, I will be computing the counts of each level of _prop_management_company_name_. I have chosen _prop_management_company_name_ since this variable appears to be a potential variable of interest for this research question as there are many variety of information in the rows from a glance. 

```{r}
#count level
#prop_management_fct <- as.factor(apt_buildings$prop_management_company_name) %>% #convert to factor
#  nlevels()

#count NA for all variables
apt_buildings %>%
  summarise_all(~sum(is.na(.)))

#count total number of rows in property_management_company_name
#apt_buildings %>%
#  select(prop_management_company_name) %>%
#  summarise(n())


#count number of NA in property_management_company_name
#apt_buildings %>%
#  select(prop_management_company_name) %>%
...
For this task, I will convert the _amenities_ variable to a factor, then compute the counts of each level. Then I will tidy the data by converting each cell into a single value.

```{r}
amenities_fct <- as.factor(apt_buildings$amenities) #convert amenities to factor

amenities_count <- fct_count(amenities_fct) #compute counts of each level
